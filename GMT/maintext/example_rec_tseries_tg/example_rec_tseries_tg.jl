...
fn_tg  = homedir() * "/Data/ECCO/RISE/results/Charleston/Charleston_ccsm_TG_tseries.nc"
dir_gmt = homedir() * "/Projects/2020_ECCO_adjoint/GMT/Charleston/maintext/example_rec_tseries_tg/"
mnum=1
...
# Save data
writedlm(dir_gmt*"obs_pred_1m.txt",[tval_1m obs_pred_1m.-msc_1m],';')
writedlm(dir_gmt*"sla_pred_1m.txt",[tval_1m sla_pred_1m.-msc_1m],';')
writedlm(dir_gmt*"obs_seas_1m.txt",[tval_1m obs_seas_1m.-msc_1m],';')
writedlm(dir_gmt*"obs_1m.txt",[tval_1m obs_1m.-msc_1m],';')

writedlm(dir_gmt*"obs_pred_4m.txt",[tval_4m obs_pred_4m.-msc_4m],';')
writedlm(dir_gmt*"sla_pred_4m.txt",[tval_4m sla_pred_4m.-msc_4m],';')
writedlm(dir_gmt*"obs_seas_4m.txt",[tval_4m obs_seas_4m.-msc_4m],';')
writedlm(dir_gmt*"obs_4m.txt",[tval_4m obs_4m.-msc_4m],';')
for i in 1:10
    writedlm(dir_gmt*"obs_pred_ens_"*string(i)*"_1m.txt",[tval_1m obs_pred_ens_1m[i,:].-msc_1m])
    writedlm(dir_gmt*"obs_pred_ens_"*string(i)*"_4m.txt",[tval_4m obs_pred_ens_4m[i,:].-msc_4m])
    writedlm(dir_gmt*"sla_pred_ens_"*string(i)*"_1m.txt",[tval_1m sla_pred_ens_1m[i,:].-msc_1m])
    writedlm(dir_gmt*"sla_pred_ens_"*string(i)*"_4m.txt",[tval_4m sla_pred_ens_4m[i,:].-msc_4m])
end
